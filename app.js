...
function applyRole(){
  ...
  if(rl) rl.textContent = (ROLE==="admin" ? "관리자 모드" : "조회 모드");
...
/* 로그아웃 → 세션 해제 후 비밀번호 화면으로 (역할 전환용) */
window.logout = ()=>{ sessionStorage.removeItem("songjeon_unlock"); sessionStorage.removeItem("songjeon_role"); location.reload(); };
